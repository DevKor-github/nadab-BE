...
public class NaverOAuth2Client {
    ...
    public String fetchAccessToken(String code, String state) {
                ...
                .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                        clientResponse -> clientResponse.bodyToMono(NaverTokenResponse.class)
                                .map(errorResponse -> {
                                    ...
                                    log.warn("네이버 토큰 발급 실패: {}", errorMessage);
        ...
        if (response.getAccessToken() == null) {
            log.warn("네이버 토큰 발급 실패: access_token 필드 없음");
            throw new OAuth2Exception(ErrorCode.AUTH_OAUTH2_TOKEN_FAILED);
        }
    ...
    public OAuth2UserInfo fetchUserInfo(String accessToken) {
                        ...
                        clientResponse -> clientResponse.bodyToMono(NaverProfileResponse.class)
                                .map(errorResponse -> {
                                    String errorMessage = errorResponse.getMessage() != null
                                            ...
                                            : "HTTP " + clientResponse.statusCode();
                                    log.warn("네이버 사용자 정보 조회 실패: {}", errorMessage);
        ...
        // 응답 없음
        if (response == null) {
            log.warn("네이버 사용자 정보 조회 실패: 응답 없음");
        ...
        if (profileData == null) {
            log.warn("네이버 사용자 정보 조회 실패: response 필드 없음");
            throw new OAuth2Exception(ErrorCode.AUTH_OAUTH2_USERINFO_FAILED);
        }

        // 필수 필드 체크
        if (profileData.getEmail() == null || profileData.getId() == null) {
            log.warn("네이버 사용자 정보 조회 실패: 필수 필드 없음");
            throw new OAuth2Exception(ErrorCode.AUTH_OAUTH2_USERINFO_FAILED);
        }
